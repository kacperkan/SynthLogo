...
def poisson_solve(gx, gy, bnd):
    ...
    D = (2 * np.cos(np.pi * xx / (W - 1)) - 2) + (
        2 * np.cos(np.pi * yy / (H - 1)) - 2
    )
    L_dst = L_dst / D
...
def blit_images(im_top, im_back, scale_grad=1.0, mode="max"):
    ...
    for ch in range(im_top.shape[2]):
        ...
        if mode == "max":
            ...
            f_gx = np.sum((gx[gxs_idx] == gxs[gxs_idx]).flat) / (
                np.sum(gxs_idx.flat) + 1e-6
            )
            f_gy = np.sum((gy[gys_idx] == gys[gys_idx]).flat) / (
                np.sum(gys_idx.flat) + 1e-6
            )
...
if __name__ == "__main__":
    ...
    mu = np.mean(
        np.reshape(im_src, [im_src.shape[0] * im_src.shape[1], 3]), axis=0
    )
    # print mu
    sz = (700, 700)
    ...
    im_actual_L = cv2.cvtColor(im_src.astype("uint8"), cv2.cv.CV_BGR2Lab)[
        :, :, 0
    ]
    im_alpha_L = cv2.cvtColor(im_alpha.astype("uint8"), cv2.cv.CV_BGR2Lab)[
        :, :, 0
    ]
    im_poisson_L = cv2.cvtColor(im_res.astype("uint8"), cv2.cv.CV_BGR2Lab)[
        :, :, 0
    ]
