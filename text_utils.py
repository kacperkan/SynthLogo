...
class RenderFont(object):
    ...
    def render_curved(self, font, word_text):
        ...
        for i in range(wl):
            ...
            if i > mid_idx:
                newrect.topleft = (
                    last_rect.topright[0] + 2,
                    newrect.topleft[1],
                )
            else:
                newrect.topright = (
                    last_rect.topleft[0] - 2,
                    newrect.topleft[1],
                )
            newrect.centery = max(
                newrect.height,
...
class FontState(object):
    ...
    def sample(self):
        ...
        return {
            ...
            "strength": (self.strength[1] - self.strength[0])
            * np.random.rand()
            + self.strength[0],
            "char_spacing": int(
                self.kerning[3]
                * (np.random.beta(self.kerning[0], self.kerning[1]))
                + self.kerning[2]
            ),
