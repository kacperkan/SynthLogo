...
def process_db_parallel(base_dir, th=0.11):
    ...
    class ucm_iterable(object):
        ...
        def get_imname(self, i):
            return "".join(
                map(chr, self.ucm_h5[self.ucm_h5["names"][0, self.i]][:])
            )
    ...
    parpool = mp.Pool(4)
    ucm_result = parpool.imap_unordered(
        get_mask_parallel, ucm_iter, chunksize=1
    )
...
base_dir = (
    "/home/"
)  # directory containing the ucm.mat, i.e., output of run_ucm.m
