...
def plot_xyzrgb(xyz, rgb, show=False):
    ...
    pts_glyph = mym.points3d(
        xyz[:, 0], xyz[:, 1], xyz[:, 2], rgb_s, mode="point"
    )
...
def visualize_plane(pt, plane, show=False):
    ...
    mym.quiver3d(
        mu[0], mu[1], mu_z, plane[0], plane[1], plane[2], scale_factor=0.3
    )
...
def implicit_plot(
    ...
    x, y, z = np.mgrid[
        xl : xr : eval("{}j".format(Nx)),
        yl : yr : eval("{}j".format(Ny)),
        zl : zr : eval("{}j".format(Nz)),
    ]
    scalars = eval(expr)
    src = mym.pipeline.scalar_field(x, y, z, scalars)
    cont1 = mym.pipeline.iso_surface(
        src, color=col_isurf, contours=[0], transparent=False, opacity=0.8
    )
    cont1.compute_normals = (
        False
    )  # for some reasons, setting this to true actually cause
