...
class Layer(object):
    def __init__(self, alpha, color):
        ...
        if color.ndim == 1:  # constant color for whole layer
            ...
            self.color = (
                np.repeat(color[:, :, None], repeats=3, axis=2)
                .copy()
                .astype("uint8")
            )
...
class FontColor(object):
    def __init__(self, col_file):
        ...
        # computations:
        self.colorsLAB = np.r_[
            self.colorsRGB[:, 0:3], self.colorsRGB[:, 6:9]
        ].astype("uint8")
        self.colorsLAB = np.squeeze(
            cv.cvtColor(self.colorsLAB[None, :, :], cv.cv.CV_RGB2Lab)
        )
    ...
    def complement(self, rgb_color):
        ...
        """
        col_hsv = np.squeeze(
            cv.cvtColor(rgb_color[None, None, :], cv.cv.CV_RGB2HSV)
        )
        col_hsv[0] = col_hsv[0] + 128  # uint8 mods to 255
        col_comp = np.squeeze(
            cv.cvtColor(col_hsv[None, None, :], cv.cv.CV_HSV2RGB)
        )
        return col_comp
...
class Colorize(object):
    ...
    def color_border(self, col_text, col_bg):
        ...
        if choice == 0:
            ...
            col_text = np.squeeze(
                cv.cvtColor(col_text[None, None, :], cv.cv.CV_HSV2RGB)
            )
        elif choice == 1:
            ...
            col_bg = np.squeeze(
                cv.cvtColor(col_bg[None, None, :], cv.cv.CV_HSV2RGB)
            )
            col_text = np.squeeze(
                cv.cvtColor(col_text[None, None, :], cv.cv.CV_HSV2RGB)
            )
        ...
        col_text = np.squeeze(
            cv.cvtColor(col_text[None, None, :], cv.cv.CV_RGB2HSV)
        )
    ...
    def process(self, text_arr, bg_arr, min_h):
        ...
        l_text.alpha = l_text.alpha * np.clip(
            0.88 + 0.1 * np.random.randn(), 0.72, 1.0
        )
        layers = [l_text]
        blends = []
        ...
        if np.random.rand() < self.p_border:
            ...
            l_border = Layer(
                border_a, self.color_border(l_text.color, l_bg.color)
            )
        ...
        if np.random.rand() < self.p_drop_shadow:
            ...
            # shadow angle:
            theta = (
                np.pi / 4 * np.random.choice([1, 3, 5, 7])
                + 0.5 * np.random.randn()
            )
    ...
    def color(self, bg_arr, text_arr, hs, place_order=None, pad=20):
        ...
        for i in place_order[::-1]:
            ...
            text_patch = np.pad(
                text_patch,
                pad_width=((num_pad[0], num_pad[2]), (num_pad[1], num_pad[3])),
                mode="constant",
            )
            l -= num_pad[:2]
